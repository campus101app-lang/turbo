...
final _requestsProvider = FutureProvider.autoDispose<List<PaymentRequest>>((ref) async {
...
class _RequestsScreenState extends ConsumerState<RequestsScreen> {
  ...
  Widget _buildError(BuildContext context, String err) {
    return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      Text('Failed to load requests', style: Theme.of(context).textTheme.bodyMedium),
  ...
  Widget _buildBody(BuildContext context, List<PaymentRequest> all) {
    ...
    final totalPending = all.where((r) => r.isPending)
        .fold<double>(0, (s, r) => s + r.amount);
    final totalPaid = all.where((r) => r.isPaid)
        .fold<double>(0, (s, r) => s + r.amount);
      ...
      child: ListView(
        ...
        children: [
          ...
          SingleChildScrollView(
            ...
            child: Row(children: ['all', 'pending', 'paid', 'expired', 'cancelled']
                .map((f) => _FilterChip(
          ...
          if (filtered.isEmpty)
            Center(
              child: Padding(
                ...
                child: Text('No ${ _filter == 'all' ? '' : _filter} requests',
  ...
  void _showCreateSheet(BuildContext context) {
    showDayFiBottomSheet(
      ...
      child: _CreateRequestSheet(onCreated: () => ref.invalidate(_requestsProvider)),
    );
  }
...
class _SummaryRow extends StatelessWidget {
  ...
  const _SummaryRow({
    required this.pending, required this.paid,
    required this.totalPending, required this.totalPaid,
  });
...
class _RequestTile extends StatelessWidget {
          ...
          Expanded(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Text(
                ...
                maxLines: 1, overflow: TextOverflow.ellipsis,
                style: GoogleFonts.outfit(fontSize: 12, color: ext.secondaryText),
              ),
            ]),
          ),
  ...
  Color _statusColor(String s) {
    switch (s) {
      ...
      case 'expired':   return DayFiColors.red;
  ...
  IconData _statusIcon(String s) {
    switch (s) {
      ...
      default:          return Icons.request_quote_rounded;
    }
  }
}
  ...
  Widget build(BuildContext context) {
    return Center(
      child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        Icon(Icons.request_quote_rounded, size: 56,
            color: Theme.of(context).colorScheme.onSurface.withOpacity(0.08)),
        const SizedBox(height: 6),
        Text('No payment requests', style: Theme.of(context).textTheme.bodyMedium),
        const SizedBox(height: 4),
        Text('Request money from clients instantly',
            style: Theme.of(context).textTheme.bodySmall?.copyWith(
  ...
  Future<void> _submit() async {
    ...
    try {
      final result = await apiService.createRequest({
        ...
        'payerEmail': _payerEmail.text.trim().isEmpty ? null : _payerEmail.text.trim(),
      });
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const SizedBox(height: 6),
        Text('A payment link will be generated to share with your client',
            style: GoogleFonts.outfit(
              fontSize: 12,
              color: Theme.of(context).colorScheme.onSurface.withOpacity(0.45),
            )),
...
class _LinkRevealSheet extends StatelessWidget {
  ...
  final double amount;
  final String asset;
  const _LinkRevealSheet({required this.link, required this.amount, required this.asset});
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        const SizedBox(height: 24),
        Icon(Icons.link_rounded, size: 48,
        ...
        Text(
          ...
          style: GoogleFonts.outfit(
            fontSize: 28, fontWeight: FontWeight.w300, letterSpacing: -1,
            color: Theme.of(context).colorScheme.primary,
          ),
        ),
        const SizedBox(height: 20),
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        if (r.payerName != null)  _DetailRow(label: 'From',  value: r.payerName!),
        if (r.payerEmail != null) _DetailRow(label: 'Email', value: r.payerEmail!),
        if (r.note != null)       _DetailRow(label: 'Note',  value: r.note!),
        _DetailRow(label: 'Created',    value: DateFormat('MMM d, yyyy').format(r.createdAt)),
        if (r.paidAt != null)
          _DetailRow(label: 'Paid at',  value: DateFormat('MMM d, yyyy').format(r.paidAt!)),
        if (r.expiresAt != null)
          _DetailRow(label: 'Expires',  value: DateFormat('MMM d, yyyy').format(r.expiresAt!)),
...
class _SegmentPicker extends StatelessWidget {
  ...
  const _SegmentPicker({required this.options, required this.selected, required this.onChanged});
